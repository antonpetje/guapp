...
GuaApp.config(['$stateProvider', '$urlRouterProvider', function($stateProvider, $urlRouterProvider) {
	$stateProvider
		...
		.state('menu.area', {url: '/area', views: {'menuContent': {templateUrl: 'areaView.html', controller: 'AreaCtrl'} }  })
		.state('menu.info', {url: '/info', views: {'menuContent': {templateUrl: 'infoView.html', controller: 'InfoCtrl'} }  })

		.state('menu.tentang', {url: '/info/tentang', views: {'menuContent': {templateUrl: 'dummyView.html', controller: 'TentangCtrl'} }  })
		.state('menu.jenis', {url: '/info/jenis', views: {'menuContent': {templateUrl: 'dummyView.html', controller: 'JenisCtrl'} }  })
		.state('menu.alat', {url: '/info/alat', views: {'menuContent': {templateUrl: 'dummyView.html', controller: 'AlatCtrl'} }  })
		.state('menu.tingkat', {url: '/info/tingkat', views: {'menuContent': {templateUrl: 'dummyView.html', controller: 'TingkatCtrl'} }  })
		.state('menu.biota', {url: '/info/biota', views: {'menuContent': {templateUrl: 'dummyView.html', controller: 'BiotaCtrl'} }  })
		.state('menu.ornamen', {url: '/info/ornamen', views: {'menuContent': {templateUrl: 'dummyView.html', controller: 'OrnamenCtrl'} }  })
 ...
 * MAIN CONTROLLER - handle inapp browser
 */
GuaApp.controller('JenisCtrl', ['$scope', function($scope) {
  // do something
}]);
 ...
 * MAIN CONTROLLER - handle inapp browser
 */
GuaApp.controller('AlatCtrl', ['$scope', function($scope) {
  // do something
}]);
 ...
 * MAIN CONTROLLER - handle inapp browser
 */
GuaApp.controller('TingkatCtrl', ['$scope', function($scope) {
  // do something
}]);
 ...
 * MAIN CONTROLLER - handle inapp browser
 */
GuaApp.controller('BiotaCtrl', ['$scope', function($scope) {
  // do something
}]);
 ...
 * MAIN CONTROLLER - handle inapp browser
 */
GuaApp.controller('OrnamenCtrl', ['$scope', function($scope) {
  // do something
}]);
 ...
 */
GuaApp.controller('GpsCtrl', ['$scope','$ionicPlatform', '$location',
	function($scope, $ionicPlatform, $location) {

	// init gps array
    $scope.whoiswhere = [];
    $scope.basel = { lat: -7.801389, lon: 110.364444 };
 ...
 * MAIN CONTROLLER - handle inapp browser
 */
GuaApp.controller('AreaCtrl', ['$scope', function($scope) {
  // do something
  $scope.listareas = [];
  $.ajax({
	  ...
	  dataType: "json",
	  type: "post",
	}).done(function listGua(larea){
		if(larea.name == "Ambolabong"){
		$.each(larea.data, function(x, list){
			$scope.listareas.push({id:list.id, name:list.name});
			
		});	
	}
		console.log($scope.listareas.name);
	});
...
GuaApp.directive("appMap", function ($window) {
    return {
        restrict: "E",
        replace: true,
        template: "<div></div>",
        ...
        link: function (scope, element, attrs) {
			...
			function createMap() {
				...
				var mapOptions = {
					zoom: 13,
					center: new google.maps.LatLng(-7.801389, 110.364444),
